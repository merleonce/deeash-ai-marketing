...
try:
    ...
    st.error(f"⚠️ ไม่พบไฟล์โมเดล AI กรุณาตรวจสอบว่าโหลดมาไว้ในโฟลเดอร์เดียวกันแล้ว\nError: {e}")
    models_loaded = False
...
if models_loaded:
    ...
    with tab1:
        ...
        if st.button("🚀 ประมวลผลกลยุทธ์", key="btn_u"):
            ...
            if online_raw == "ออนไลน์ (Shopee, TikTok)":
                ...
                promo_msg = "O2O Trigger: ปกติซื้อหน้าร้านใช่ไหม? ลองสั่งผ่าน Shopee/TikTok ครั้งแรกวันนี้! รับโค้ดส่งฟรีและลดเพิ่ม 30%"

            st.success(f"**Persona:** {result['title']}")
            ...
            st.info(f"💬 **ข้อความโฆษณา:**\n{final_msg}")
            st.warning(f"🎁 **โปรโมชัน O2O แนะนำ:**\n{promo_msg}")
    ...
    with tab2:
        ...
        if st.button("🚀 ประมวลผลกลยุทธ์", key="btn_nu"):
            ...
            st.success(f"**Persona:** {result2['title']}")
